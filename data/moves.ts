...
export const Moves: {[moveid: string]: MoveData} = {
	...
	aerialace: {
		...
		pp: 20,
		priority: 0,
		flags: {blade: 1, contact: 1, protect: 1, mirror: 1, distance: 1},
		secondary: null,
		target: "any",
		type: "Flying",
		contestType: "Cool",
	...
	airslash: {
		...
		pp: 15,
		priority: 0,
		flags: {blade: 1, protect: 1, mirror: 1, distance: 1},
		secondary: {
			chance: 30,
	...
	brickbreak: {
		...
		onTryHit(pokemon) {
			...
			pokemon.side.removeSideCondition('rainbowwall');
		},
		secondary: null,
		target: "normal",
		type: "Fighting",
	...
	coaching: {
		...
		pp: 10,
		priority: 0,
		flags: {bypasssub: 1, allyanim: 1, sound: 1},
		secondary: null,
		boosts: {
			atk: 1,
			def: 1,
	...
	courtchange: {
		...
		onHitField(target, source) {
			const sideConditions = [
				'mist', 'lightscreen', 'reflect', 'spikes', 'safeguard', 'tailwind', 'toxicspikes', 'stealthrock', 'waterpledge', 'firepledge', 'grasspledge', 'stickyweb', 'auroraveil', 'gmaxsteelsurge', 'gmaxcannonade', 'gmaxvinelash', 'gmaxwildfire', 'rainbowwall', 'blackice', 'healingcircle', 'livewire', 'eggcluster', 'sturdyspikes',
			];
			let success = false;
	...
	defog: {
		...
		onHit(target, source, move) {
			...
			const removeTarget = [
				'reflect', 'lightscreen', 'auroraveil', 'safeguard', 'mist', 'spikes', 'toxicspikes', 'stealthrock', 'stickyweb', 'gmaxsteelsurge', 'rainbowwall', 'blackice', 'healingcircle', 'livewire', 'eggcluster',
			];
			const removeAll = [
				'spikes', 'toxicspikes', 'stealthrock', 'stickyweb', 'gmaxsteelsurge', 'blackice', 'healingcircle', 'livewire', 'eggcluster',
			];
	...
	dive: {
		...
		pp: 15,
		priority: 0,
		flags: {contact: 1, charge: 1, mirror: 1, nonsky: 1, allyanim: 1},
		breaksProtect: true,
		onTryMove(attacker, defender, move) {
			if (attacker.removeVolatile(move.id)) {
				return;
	...
	echoedvoice: {
		...
		basePowerCallback() {
			if (this.field.pseudoWeather.echoedvoice) {
				return 45 * this.field.pseudoWeather.echoedvoice.multiplier;
			}
			return 45;
		},
		category: "Special",
	...
	encore: {
		...
		pp: 5,
		priority: 0,
		flags: {protect: 1, reflectable: 1, mirror: 1, bypasssub: 1, sound: 1},
	...
	floralhealing: {
		...
		onHit(target, source) {
			let success = false;
			if (this.field.isTerrain('grassyterrain') || this.field.isTerrain('mistyterrain')) {
	...
	flowershield: {
		...
		onHitField(t, source, move) {
			...
			for (const target of targets) {
				success = this.boost({def: 2}, target, source, move) || success;
			}
			return success;
		},
		secondary: null,
	...
	magicpowder: {
		...
		pp: 20,
		priority: 0,
		flags: {powder: 1, protect: 1, reflectable: 1, mirror: 1, allyanim: 1, magic: 1},
		onHit(target) {
	...
	metronome: {
		...
		flags: {},
		noMetronome: [
			"After You", "Assist", "Aura Wheel", "Baneful Bunker", "Beak Blast", "Belch", "Bestow", "Celebrate", "Chatter", "Copycat", "Counter", "Crafty Shield", "Decorate", "Destiny Bond", "Detect", "Endure", "Feint", "Focus Punch", "Follow Me", "Helping Hand", "Hold Hands", "Instruct", "King's Shield", "Mat Block", "Me First", "Metronome", "Mimic", "Mirror Coat", "Mirror Move", "Nature Power", "Obstruct", "Protect", "Quash", "Quick Guard", "Rage Powder", "Sketch", "Sleep Talk", "Snatch", "Snore", "Spiky Shield", "Spotlight", "Struggle", "Switcheroo", "Transform", "Trick", "Wide Guard",
		],
	...
	pollenpuff: {
		...
		pp: 20,
		priority: 0,
		flags: {bullet: 1, protect: 1, mirror: 1, allyanim: 1, pulse: 1},
		onTryHit(target, source, move) {
	...
	punishment: {
		...
		basePower: 0,
		basePowerCallback(pokemon, target) {
			let power = 60 + 30 * target.positiveBoosts();
			return power;
		},
		category: "Physical",
		name: "Punishment",
	...
	purify: {
		...
		onHit(target, source) {
			...
			this.heal(Math.ceil(source.maxhp * 0.75), source);
		},
		secondary: null,
		target: "normal",
		type: "Poison",
	...
	rapidspin: {
		...
		onAfterHit(target, pokemon) {
			...
			if (pokemon.hp && pokemon.removeVolatile('reapermark')) {
				this.add('-end', pokemon, 'Reaper Mark', '[from] move: Rapid Spin', '[of] ' + pokemon);
			}
			const sideConditions = ['spikes', 'toxicspikes', 'stealthrock', 'stickyweb', 'gmaxsteelsurge', 'blackice', 'livewire', 'eggcluster'];
	...
	rockwrecker: {
		...
		pp: 5,
		priority: 0,
		flags: {bullet: 1, recharge: 1, protect: 1, mirror: 1, pulse: 1},
		self: {
			volatileStatus: 'mustrecharge',
		},
		secondary: {
	...
	solarblade: {
		...
		pp: 10,
		priority: 0,
		flags: {blade: 1, contact: 1, charge: 1, protect: 1, mirror: 1},
		onTryMove(attacker, defender, move) {
			if (attacker.removeVolatile(move.id)) {
				return;
			}
	...
	spitup: {
		...
		basePowerCallback(pokemon) {
			...
			return pokemon.volatiles['stockpile'].layers * 150;
		},
		category: "Special",
		name: "Spit Up",
		pp: 15,
	...
	timberfall: {
		...
		basePowerCallback(pokemon, target, move) {
			let bp = move.basePower;
			if (pokemon.volatiles['timberfall'] && pokemon.volatiles['timberfall'].hitCount) {
				bp *= Math.pow(2, pokemon.volatiles['timberfall'].hitCount);
			}
			if (pokemon.status !== 'slp') pokemon.addVolatile('timberfall');
			if (pokemon.volatiles['defensecurl']) {
				bp *= 2;
			}
			this.debug("Timberfall bp: " + bp);
			return bp;
		},
		category: "Physical",
		name: "Timberfall",
		...
		condition: {
			...
			onResidual(target) {
				if (target.lastMove && target.lastMove.id === 'struggle') {
					// don't lock
					delete target.volatiles['timberfall'];
				}
			},
		},
		secondary: null,
	...
	faetality: {
		...
		onHit(target, source, move) {
			if (!source.isAlly(target)) this.hint(move.category + " Faetality");
		},
		onAfterSubDamage(damage, target, source, move) {
			if (!source.isAlly(target)) this.hint(move.category + " Faetality");
		},
